  ...
  async receiveMessage(message) {
    ...
    if (message.isResponseFlag) {
      ...
      if (message.api === "searchDevice") {
        const result = await window.searchDevice();
        this._postMessage(message.api, result, message.callbackId, true);
      } else if (message.api === "getFeatures") {
        const result = await window.getFeatures(message.data);
        this._postMessage(message.api, result, message.callbackId, true);
      } else if (message.api === "btcGetAddress") {
        const result = await window.btcGetAddress(message.data);
        this._postMessage(message.api, result, message.callbackId, true);
      } else if (message.api === "monitorCharacteristic") {
        const result = await window.monitorCharacteristic(message.data);
        this._postMessage(message.api, result, message.callbackId, true);
      } else {
        // 对为支持的api返回默认null
